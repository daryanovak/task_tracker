...
class TaskControllerTestCase(unittest.TestCase):
    ...
    def test_cron_periodic_task(self):
        with self.assertRaises(errs.CronValueError):
            self.controller.create_periodic_task(title="title", text="text", status=1,start_date="01/01/12 00:00",
                                                 date="01/01/11 00:00", period="* help* * * *", parent_id=3)
    ...
    def test_delete_not_exist_task(self):
        with self.assertRaises(errs.TaskNotExistError):
            self.controller.delete_task(task_id=1)

    def test_get_task_by_id(self):
        with self.assertRaises(errs.TaskNotExistError):
            self.controller.get_task_by_id(task_id=1)

    def test_get_task_by_id_(self):
        with self.assertRaises(errs.TaskNotExistError):
            self.controller.get_task_by_id(task_id=-323)

    def test_edit_task_title(self):
        with self.assertRaises(errs.InvalidTypeParameterError):
            self.controller.edit_task_title(task_id=1, new_title="")
